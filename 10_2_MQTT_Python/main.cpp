...
int16_t pDataXYZ[3] = {0};  // create a array to store XYZ
...
void publish_message(MQTT::Client<MQTTNetwork, Countdown>* client) {
    BSP_ACCELERO_AccGetXYZ(pDataXYZ);   // get XYZ data from accelerometer
    message_num++;
    MQTT::Message message;
    char buff[100];
    sprintf(buff, "%d, %d, %d #%d", pDataXYZ[0], pDataXYZ[1], pDataXYZ[2], message_num);    // new sprintf
    message.qos = MQTT::QOS0;
    message.retained = false;
    message.dup = false;
    message.payload = (void*) buff;
